...
static mjs_val_t do_op(struct mjs *mjs, mjs_val_t a, mjs_val_t b, int op) {
  ...
  if ((mjs_is_foreign(a) || mjs_is_number(a)) &&
    ...
    ret = s_concat(mjs, a, b);
		} else if (mjs_is_string(a) && mjs_is_number(b) && (op == TOK_PLUS)) {
			char *p = NULL;
			int needFree = 0;
			size_t len = 0;
			mjs_err_t err = mjs_to_string(mjs, &b, &p, &len, &needFree);
			mjs_val_t bStr = mjs_mk_string(mjs, p, len, 1);
			ret = s_concat(mjs, a, bStr);
			if (needFree) free(p);
		} else if (mjs_is_number(a) && mjs_is_string(b) && (op == TOK_PLUS)) {
			char *p = NULL;
			int needFree = 0;
			size_t len = 0;
			mjs_err_t err = mjs_to_string(mjs, &a, &p, &len, &needFree);
			mjs_val_t aStr = mjs_mk_string(mjs, p, len, 1);
			ret = s_concat(mjs, aStr, b);
			if (needFree) free(p);
		} else {
			printf("readout: %d %d %d %d\n", mjs_is_number(a), mjs_is_number(b), mjs_is_string(a), mjs_is_string(b));

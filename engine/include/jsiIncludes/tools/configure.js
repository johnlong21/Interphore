...
function configure(inp:array):number {
        ...
        for (var ai=0; ai<input.length; ai++) {
            ...
            switch (cpre) {
                ...
                case 'reconfig': // Regenerate files in Configs dir.
                    var flst = File.glob('Configs/make_*.conf');
                    for (var fi of flst) {
                        var fnv = File.rootname(File.tail(fi)).split('_')[1];
                        puts("DOING: "+fnv);
    ...
    function outResult() {
        var confStr='', INARGS = '', CCDEFS='', MODDEFS='',
            MKOPTS="JSI_CONFIG_DEFINED=1\n";
        
        for (var i in List.allopt) {
            var copt = List.allopt[i];
            var weup=copt.toUpperCase();
            var hv = 0;
        ...
        for (var i in List.buildin) {
            var val= List.buildin[i];
            if (List.ext.indexOf(val)<0)
                List.ext.push(val);
        }
        for (var i in List.ext) {
            var copt = List.ext[i], weup=copt.toUpperCase();
        ...
        for (var i in List.mod) {
            var copt = List.mod[i], weup=copt.toUpperCase();
        ...
        for (var i in List.buildin) {
            var copt = List.buildin[i], weup=copt.toUpperCase();
